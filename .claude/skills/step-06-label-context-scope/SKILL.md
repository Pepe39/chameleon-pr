...
1. **DIFF** — Could the comment be made by reading only the changed lines in the PR?
   ...
   - **New files rule:** If the target file is completely new in the PR (diff header `@@ -0,0 +1,N @@`, all lines are added), the scope is `diff` even if you need to read multiple parts of the file. There is no `file` scope for a file that did not exist before the PR.
...
- **Do NOT choose `file` for new files.** If the target file is completely new in the PR, everything is diff. There is no file scope for files that did not exist before the PR.
- **Do NOT choose `file` just because the file is large.** The file's size is irrelevant. What matters is whether the lines you need are in the diff or not.
- **Do NOT choose `repo` because "Copilot must have read other files."** Do not speculate about what Copilot consumed. Evaluate what information was needed to make the comment.
